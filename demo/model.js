...
(function(global) {
  ...
  function getVAEModelParams(data) {
    ...
    dec_fc_kernel = dl.Array2D.new([z_size, 1024], dequantize(data[0], factor)); //dequantize is dividing by factor
...
(function(global) {
  ...
  var init_mu=[ 77, -195, -2838, 342, 126, 5553, -14870, -9004, 49, 20, -198, -106, 1798, -169, 104, 11257, -306, 241, -14165, -28, 63, 168, 9151, -77, 12211, -146, 106, 14, 9587, 109, 6, 115, 171, 74, 161, 6171, 257, 19, -61, -214, 18330, 1496, -122, 38, 7812, -110, -183, -3, 24, 2757, -250, -10229, -97, -5783, 148, -3409, 84, -211, -245, -125, -12217, -234, -81, 246 ];
  var init_logvar=[ 64, 21, -28010, -18, 121, -3016, -28438, 26996, 188, -56, -174, 65, -11955, -98, -22, -23914, -263, -25706, 13157, -66, 54, 16, 32347, -328, -4581, -130, 41, -34, -15155, -36, -192, -238, -181, 34, -16, 12825, -275, 171, 167, -270, -25951, -27722, -49, -94, -16695, -38, -214, -20, -20, 21567, -155, -32375, 14, 13960, -55, -23677, -87, -399, -9, -20122, -26868, 123, 48, -117 ];
  ...
  function getVAEModelParams(data) {
    ...
    var factor = 10000;
    /*
    dec_fc/kernel:0 (64, 1024)
    dec_fc/bias:0 (1024,)
    dec_deconv1/kernel:0 (5, 5, 128, 1024)
    dec_deconv1/bias:0 (128,)
    dec_deconv2/kernel:0 (5, 5, 64, 128)
    dec_deconv2/bias:0 (64,)
    dec_deconv3/kernel:0 (6, 6, 32, 64)
    dec_deconv3/bias:0 (32,)
    dec_deconv4/kernel:0 (6, 6, 3, 32)
    dec_deconv4/bias:0 (3,)
    */
    //console.log('data[0]', data[0])
    //console.log('dequantize', dequantize(data[0], factor))
  ...
  function vae_decode(z_array) {
    //console.log('vae decode', z_array)
    var out = math.scope(function(keep, track) { // keep and track are two methods to help with memory management
      // Because you are constructing an array directly,
      // our automatic memory manager doesn't know about it. You should "track" it.
      // Later versions will fix this problem and do this automatically, we're working on it
      ...
      var h = math.add(math.matMul(dec_z.reshape([1, z_size]), dec_fc_kernel), dec_fc_bias);
      h = h.reshape([1, 1, 4*256]); //h = h.reshape([1, 1, 4*256]);
      h = math.relu(math.add(math.conv2dTranspose(h, dec_deconv1_kernel, [5, 5, 128], 2, "valid"), dec_deconv1_bias));
      h = math.relu(math.add(math.conv2dTranspose(h, dec_deconv2_kernel, [13, 13, 64], 2, "valid"), dec_deconv2_bias));
      h = math.relu(math.add(math.conv2dTranspose(h, dec_deconv3_kernel, [30, 30, 32], 2, "valid"), dec_deconv3_bias));
      h = math.sigmoid(math.add(math.conv2dTranspose(h, dec_deconv4_kernel, [64, 64, 3], 2, "valid"), dec_deconv4_bias));
  ...
  function process_b64datacheck(data) {
    var result = [];
    for (var i=0;i<data.length;i++) {
      var blob = string_to_array1(data[i]);
  ...
  //console.log('carvae_data_before', carvae_data)
  //carvae_data = process_b64data(carvae_data, 1); //*=factor
  //console.log('carvae_data',carvae_data)
