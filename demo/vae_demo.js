  ...
  var vae_sketch = function( p ) { 
    ...
    function load_latent_json() {
      $.getJSON( settings.data_dir+file_number+".json", function( data ) {
        var i;
        z = new Array(z_size);
        console.log(file_number, data)
        if (data[0].length > 1) { // I'm so stupid i saved data in different format
          // carracing
          console.log('carracing')
          mu =   dequantize(data[0], 10000); //dequantize(data[0], 10000); //data[0]; //
          sigma = dequantize(data[1], 10000); //dequantize(data[1], 10000); //data[1]; //
        } else {
          // doom
    ...
    function decode(z) {
      var x = model.vae_decode(z)
      //console.log('x: ', x) //12288 size
      return pic2img(x);
    }
